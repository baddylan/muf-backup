...
$ifdef __muck=FuzzyLogic
 $define AUTOLIST "@260" $enddef
$else
 $define AUTOLIST "@260" $enddef
$endif
...
$ifdef __muck=FuzzyLogic
 $define DEFAULTPARENT #0 $enddef
$else
 $define DEFAULTPARENT #0 $enddef
$endif
  ( set this to the dbref # of the action editor program )
$ifdef __muck=FuzzyLogic
 $define ACTIONEDITOR #848 $enddef
$else
 $define ACTIONEDITOR #848 $enddef
$endif
...
$endif 
  ...
  me @ pennies ROOMCOST < if
    me @ "Sorry... you cannot afford another room.  They cost " me @ pennies intostr strcat " to build." strcat notify
    0 exit
  then
...
( --------------------------------------------------------------- )
: header ( -- )
  me @ "Room Hammer V1.4a (C) 1994 by Triggur" notify
  me @ "------------------------------------" notify
  me @ " " notify
  exit
;
...
: show-room ( -- )
  ...
  me @ "4. Throwing is" dbnum @ "j" flag? not if " NOT" strcat then " allowed." strcat notify
