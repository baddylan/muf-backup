...
: fetch
  ...
  then
  dup player? if
    "That is not a valid container object, sorry." .tell exit
  then
  dup ok? not if
    "You cannot fetch objects from there." .tell exit
  then
  dup getlockstr
  dup "*UNLOCKED*" strcmp not if pop "me" then 
  parselock me @ swap testlock not if
    dup fail .tell dup ofail .otell exit then
(
